...
-- Set up BankRewards API configuration
ALTER DATABASE postgres SET "app.settings.bankrewards_api_url" = 'https://api.bankrewards.io/v1';
ALTER DATABASE postgres SET "app.settings.bankrewards_api_key" = current_setting('app.settings.bankrewards_api_key', true);
...
-- Create get_bank_rewards_offers function
CREATE OR REPLACE FUNCTION get_bank_rewards_offers()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  api_response jsonb;
  api_url text := current_setting('app.settings.bankrewards_api_url', true);
  api_key text := current_setting('app.settings.bankrewards_api_key', true);
BEGIN
  -- Make HTTP request to BankRewards API
...
-- Grant execute permission on get_bank_rewards_offers to authenticated users
GRANT EXECUTE ON FUNCTION get_bank_rewards_offers() TO authenticated;
