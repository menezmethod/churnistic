  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      try {
        ...
        setIsLoading(false);
        console.log('[useSession] Session check completed, isAuthenticated:', isAuthenticated);
      }
    };
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event: string, session: Session | null) => {
        console.log('[useSession] Auth state changed:', event, 'Session exists:', !!session);
        
        if (session) {
          try {
            // Get user profile data on auth state change
            console.log('[useSession] Fetching user profile on auth change for ID:', session.user.id);
            const { data: userData, error: userError } = await supabase
              .from('users')
              .select('*')
            ...
            if (userError) {
              console.error('[useSession] Error getting user data on auth change:', userError);
              
              // If profile doesn't exist, create one with default values
              console.log('[useSession] Attempting to create user profile on auth change');
              const { data: newProfile, error: createError } = await supabase
                .from('users')
                .upsert([
                  {
              ...
              if (createError) {
                console.error('[useSession] Error creating user profile on auth change:', createError);
              } else if (newProfile) {
                // Combine auth user with new profile data
                const fullUser = {
                  ...session.user,
                ...
                console.log('[useSession] Setting user with new profile on auth change:', {
                  id: fullUser.id,
                  email: fullUser.email,
                  role: fullUser.role,
                  display_name: fullUser.display_name
            ...
            console.log('[useSession] Setting user with existing profile on auth change:', {
              id: fullUser.id,
              email: fullUser.email,
              role: fullUser.role,
              display_name: fullUser.display_name
