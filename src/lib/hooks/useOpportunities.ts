  ...
  try {
    ...
    if (cursor) {
      ...
      const sortColumn: string =
        sortBy === 'date' ? 'created_at' : sortBy || 'created_at';
    ...
    if (sortBy) {
      ...
      if (sortBy === 'date') {
        ...
        sortColumn =
          tableInfo && tableInfo.length > 0 && 'createdat' in tableInfo[0]
            ? 'createdat'
            : 'created_at';
      }
...
export function useOpportunities(initialParams?: Partial<OpportunitiesParams>) {
  ...
  const [filters, setFilters] = useState<OpportunitiesParams>({
    ...
    sortBy:
      (searchParams.get('sortBy') as OpportunitiesParams['sortBy']) ||
    ...
    sortDirection:
      ...
      'desc',
    status:
      searchParams.get('status') || initialParams?.status || 'approved,staged,pending',
    limit: initialParams?.limit || PAGE_SIZE,
  });

  // Define handleSubscriptionError first to avoid using it before declaration
    ...
    try {
      ...
      const subscription = supabaseClient
        ...
        .on(
          ...
          (payload: RealtimePostgresChangesPayload<Opportunity>) => {
  ...
  }, [queryClient, supabaseClient, isOffline, isSubscribed, handleSubscriptionError]);

  // Store the current setupRealtimeSubscription function in a ref to avoid circular dependencies
  useEffect(() => {
  ...
  const updateFilters = useCallback(
    (newFilters: Partial<OpportunitiesParams>) => {
  ...
  const updateMutation = useMutation({
    ...
    onError: (err, { id }, context) => {
      ...
      if (context?.previousData) {
        queryClient.setQueryData(['opportunities', queryParams], context.previousData);
      }
    ...
    onSuccess: (updatedOpportunity) => {
      ...
      queryClient.setQueryData(
        ['opportunity', updatedOpportunity.id],
        updatedOpportunity
      );
      // Then invalidate the list query to ensure it's up to date
      queryClient.invalidateQueries({ queryKey: ['opportunities'] });
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (id) => {
      ...
      const deletedOpportunity = queryClient.getQueryData<Opportunity>([
        'opportunity',
        id,
      ]);
