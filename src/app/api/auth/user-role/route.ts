...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!userProfile && user.email) {
      console.log('[user-role API] No profile found by ID, trying to find by email with case insensitivity:', user.email);
      ...
      if (emailProfile) {
        console.log('[user-role API] Found profile by email, updating ID to match auth user');
        ...
        if (updateError) {
          ...
          console.log('[user-role API] Falling back to returning role from existing profile:', emailProfile.role);
          return NextResponse.json({
            role: emailProfile.role || 'user',
            permissions: [],
          });
        } else if (updatedProfile) {
          console.log('[user-role API] Successfully updated profile ID, returning role:', updatedProfile.role);
          return NextResponse.json({
      ...
      try {
        console.log('[user-role API] No profile found, attempting force create for:', user.id);
        
        // First, try to delete any conflicting records (only in development)
        if (process.env.NODE_ENV === 'development') {
          console.log('[user-role API] In development: attempting to remove conflicting records');
          ...
          if (existingUsers && existingUsers.length > 0) {
            ...
            console.log('[user-role API] Found existing user with same email, using their role:', existingUser.role);
        ...
        const userData = {
          ...
          email: user.email || '',
          role: 'user',
          display_name: user.user_metadata?.name || user.user_metadata?.full_name || (user.email ? user.email.split('@')[0] : 'User'),
        ...
        if (insertError) {
          ...
          if (insertError.message?.includes('duplicate key') && insertError.message?.includes('users_email_key')) {
            console.log('[user-role API] Duplicate email detected, attempting to find existing user');
            ...
            if (existingUser) {
              console.log('[user-role API] Found existing user with email:', existingUser);
              return NextResponse.json({
                role: existingUser.role || 'user',
                permissions: [],
                source: 'existing_email_user',
        ...
        console.log('[user-role API] Returning newly created profile role:', newProfile?.role || 'user');
        return NextResponse.json({
    ...
    console.log('[user-role API] Returning existing profile role:', userProfile.role || 'user');
    return NextResponse.json({
