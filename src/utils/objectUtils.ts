...
function isPreservedObjectKey(key: string): boolean {
  const preservedKeys = [
    'metadata', 'details', 'user', 'preferences', 'bonus', 'source', 
    'processing_status', 'ai_insights', 'logo', 'availability', 'requirements'
  ];
  
  return preservedKeys.some(preservedKey => 
    key === preservedKey || key.endsWith(`.${preservedKey}`)
  );
}
...
export function processNestedUpdates(
  ...
  if (depth > maxDepth) {
    console.warn(`Maximum nesting depth (${maxDepth}) exceeded in processNestedUpdates for prefix: ${prefix}`);
    return { [prefix]: data };
  }
    ...
    if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
      ...
      if (isPreservedObjectKey(fullKey)) {
        ...
        if (existingValue && typeof existingValue === 'object' && !Array.isArray(existingValue)) {
          ...
          console.log(`Deep merging existing data for key: ${fullKey}`, { existingValue });
